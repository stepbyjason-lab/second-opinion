...
### 명시적 plan/review mode

`--mode plan|review`는 caller가 요청할 때만 사용한다. 두 값 모두 AGY native
`--mode plan`으로 번역되고 `--dangerously-skip-permissions`는 제거된다. 실제 `--cwd`가
...
짧은 `--timeout`을 주지 않는다** — 30분을 넘는 리뷰도 있다. 오래 걸리는 호출은
