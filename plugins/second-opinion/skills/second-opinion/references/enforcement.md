...
if (detectDirectInference(cmd)) {                   // 벤더명(truthy) 반환 시 차단, null이면 통과
  process.stderr.write("Direct codex/agy/grok call blocked in THIS project — route through your dispatcher.\n");
  process.exit(2);
}
process.exit(0);
```
...
이 훅은 **그 프로젝트에서만** 발동한다. 다른 프로젝트의 codex/agy/grok 직접호출은 건드리지 않는다 — 그게 중개자와 caller-강제의 올바른 스코프다.
