...
export function usageText() {
  ...
  return [
    ...
    "  On agy 1.1.26 or later, reasoning effort is its own axis: pass --model gemini-3.8-flash",
    "    --effort low|medium|high. The older -high suffix still resolves alone, but combining it with --effort",
    ...
    "  `vendorUsage: null` is therefore not evidence that the vendor did not run — read",
    "  `vendorUsageStatus` to tell the two apart.",
    ...
    "  vendor that stalls burns the whole hour before anything interrupts it. A growing --err file is",
    "  the liveness signal; nothing else will cut in. Long work is normal here, so judge by --err, not",
    "  elapsed time.",
    ...
    "  also gets a shell so it can read git history and run tests. One allow rule rides with it, node",
    "  (Bash(node *), PowerShell(node *)); that rule grants and does not confine. No rule list confines the",
    "  shell and node runs any script, so the brief's own prohibitions are the guard and a caller who needs the",
    "  shell gone uses --no-host-shell, which removes it and the rule. The child's dontAsk policy sorts shell",
    "  commands by shape (claude 2.1.283): plain read commands run, also chained with ; or piped or with 2>&1",
    "  and on paths outside cwd, and so does node; git -C and PowerShell script syntax (variable assignment,",
    "  foreach, a parenthesized expression, $() substitution) are refused, and one such piece refuses the whole",
    "  line. Tell a claude reviewer this in the brief: a refusal covers that command, not the shell; rewrite it",
    "  in plain form and do sums such as hashes and line counts with node -e. A claude child also",
    ...
    "  that blocks the write tools and returns the reason to the child while exec stays open, so the reviewer",
    "  runs git and tests itself. A running shell can still write, so the brief's prohibitions stay the guard.",
    "  Always-on instructions remain.",
