...
export function usageText() {
  ...
  return [
    ...
    "  (Bash(node *), PowerShell(node *)), because the child refuses node under dontAsk; that rule grants and",
    "  does not confine. No rule list confines the shell: allow rules were measured not to confine the tool",
    "  at all, and node runs any script, so the brief's own prohibitions are the guard and a caller who",
    "  needs the shell gone uses --no-host-shell, which removes it and the rule. A claude child also",
