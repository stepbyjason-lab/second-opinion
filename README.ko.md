...
- **실행 영수증** — 벤더를 부른 뒤 관측한 것을 한 줄로 남긴다: 요청한 벤더·모델,
  ...
  **프로세스가 실제로 떴는지** 여부. Codex 호출은 Codex 자신의 세션 로그에서 읽은
  ...
  순서 있는 목록을 **반복 플래그로 옮겨 적지 않고 파일째 넘기기 위한 것**이다. 두 형태를 한 호출에
  함께 줄 수는 없다 — token 출처는 호출당 하나이며, 그래야 적혀 있던 순서와 사람이 친 순서가
  섞이지 않는다. token이 거절되면 **실제로 준 플래그 이름과 몇 번째 token이 무엇인지**를 함께 낸다.
  모든 token을 stdout에서
  ...
  선택적 `--expect-total <n>`은 **전체 구획 수를 신고**한다(1~1000, 두 형태 중 어느 쪽으로든 기대 token을
  최소 하나 요구한다. **파일 줄 수로 자동 설정하지 않는다** — 그러면 전건 등록과 부분 등록이 항상 같아져
  이 값이 가르려던 구별 자체가 사라진다).
  ...
  **디스패처가 소유한 locator 필드**가 구조적으로 배제된다. `hostIsolation`의 dispatcher 소유
  ...
  방법론(Madi 등)이 영수증을 대조해 판정한다. 호출자의 설정은 **두 경로**로 벤더 자식에게
  간다 — 라이프사이클 훅이 이벤트마다 맥락을 밀어넣는 경로와, 벤더 CLI가 스스로 읽어
