...
| A pinned `--vendor` used to hand the vendor whatever `--model` string the caller typed, so a name that vendor does not publish passed `--dry-run` and failed only once the call was paid for | resolves `--model` through that one vendor's catalog too (`--vendor agy --model "opus 4.6"` → `claude-opus-4-6-thinking`). It reads only what is already cached — no provider process, no refresh — and forwards the caller's exact string whenever that is not enough to name one model, so a correct name is never renamed |
