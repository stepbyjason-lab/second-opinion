...
> Release bookkeeping: when `plugin.json` advances before its release entry,
...
- `--vendor`를 박은 호출도 **그 한 벤더의 카탈로그로** 모델 이름을 해석한다. 지금까지는 codex만
  해석했고 나머지는 호출자가 쓴 문자열이 검증 없이 벤더까지 가서, 그 벤더가 게시하지 않는 이름이
  `--dry-run`을 통과하고 실호출에서야 거절됐다. `--vendor agy --model "opus 4.6"`이 agy가 게시하는
  slug로 바뀌어 나간다.
- **뒤따르는 토큰은 다른 모델이므로 갈아끼우지 않는다.** 카탈로그 이름이 호출자가 쓴 이름 **앞에**
  토큰을 더 가진 것(`anthropic/claude-opus-4-6`)은 같은 모델이라 해석하지만, **뒤에** 더 가진
  것(`claude-sonnet-4` → `claude-sonnet-4-6`, `gpt-oss-120b` → `gpt-oss-120b-medium`)은 부르지 않은
  모델이라 후보에서 뺀다. 두 방향이 한 조건에 합쳐져 있어 구별되지 않던 자리를 갈랐다.
- 박은 경로는 **이미 있는 것만 읽는다.** 공급자 프로세스를 띄우지 않고 캐시를 갱신하지도 않으므로,
  카탈로그가 없거나 낡았거나 후보가 유일하지 않으면 호출자가 쓴 문자열을 그대로 넘긴다. 맞게 쓴
  이름이 다른 모델로 바뀌는 일은 없고, 영수증은 요청한 이름(`modelRequested`)과 실제 쓴
  이름(`model`)을 계속 따로 남긴다.
- `--vendor`를 생략한 자동 라우팅은 그대로다. 순위 취급을 바꾸지 않았다.
- **도움말이 `--mode` 생략을 가르친다.** 형제 플래그 `--operation`은 생략이 무슨 뜻인지 `--brief`
  옆에서 가르치는데 `--mode`는 말하지 않아, 그 줄을 읽은 호출자가 `--mode default`를 쓰고 검증
  오류를 받았다(실제로 발생). 받는 값이 `plan`·`review` 둘뿐이고 전권 호출은 플래그를 빼는
  것이라는 사실을 도움말과 SKILL에 적었다.

## 0.9.17 — 2026-09-13

- `--operation`을 생략하면 `text`다. 텍스트 호출이 압도적으로 많은데 매번 적어야 했고, 그 자리에서
  발주가 막힌 적이 있다. 생략을 이미지로 읽는 경로는 없다 — `--operation` 없이 `--input`을 주면
  추론하지 않고 거절한다. `image-generate`는 텍스트 호출과 구별할 표지가 없으므로 여전히 명시해야
  한다. `--operation`을 주던 기존 호출은 argv·영수증이 그대로다.
- `--expect-output-file <경로>`로 기대 토큰을 파일에서 읽는다. 한 줄에 토큰 하나이며 BOM·CRLF·줄
  앞뒤 공백·빈 줄을 받아들이고 토큰이 하나도 없으면 거절한다. 생성기가 낸 순서 있는 목록을 반복
  플래그로 옮겨 적다 순서가 뒤집히던 사고를 없애기 위한 것이라, `--expect-output`과 한 호출에 함께
  줄 수 없다 — 출처가 둘이면 순서가 다시 섞인다. 검증은 두 형태가 같은 코드를 탄다.
- `--expect-total`은 그대로 호출자가 신고한다. 파일 줄 수로 자동 설정하지 않는다 — 그러면 전건
  등록과 부분 등록이 항상 같아져 이 값이 가르려던 구별이 사라진다.
- 기대 토큰이 거절될 때 **실제로 준 플래그 이름과 몇 번째 토큰이 무엇인지**를 함께 낸다. 열두 줄
  파일에서 한 토큰이 틀렸을 때 호출자가 이분 탐색을 하지 않아도 된다. 반복 플래그로 준 호출도 같다.
- 토큰 파일도 `--brief`·`--input`과 같은 별칭 보호를 받는다. `--out`·`--err`·영수증 sink와 같은
  파일이면 출력을 열기 전에 `exit 2`다. 호출자가 넘긴 파일과 디스패처가 쓰는 파일의 목록을 한 곳으로
  모아, 새 입력이 한 검사에서만 보호받고 다른 검사에서 빠지는 모양을 구조적으로 없앴다.
